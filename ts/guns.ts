...
export class Gun extends Box {
  ...
  constructor(level: Level, x: number, y: number, weight: number) {
    super(
      ...
      level.tileSize,
      level.tileSize,
      `rgb(0, ${Math.round(255 - (weight * 255) / 100)}, 255)`
    );
    ...
    if (weight > 70) this.audio = new Audio("./sounds/heavy-shot.mp3");
    else if (weight > 30) this.audio = new Audio("./sounds/medium-shot.mp3");
    else this.audio = new Audio("./sounds/light-shot.mp3");
  }
